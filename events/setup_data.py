...
def get_setup_data():
    ...
    attendee_user = Account.objects.create_user(
    ...
    organizer_user = Account.objects.create_user(
    ...
    response = Client().post(
        path=reverse("user_login"), data=admin_data, content_type="application/json"
    )
    admin_token = response.json()["token"]

    response = Client().post(
        path=reverse("user_login"), data=attendee_data, content_type="application/json"
    )
    attendee_token = response.json()["token"]

    response = Client().post(
        path=reverse("user_login"), data=organizer_data, content_type="application/json"
    )
